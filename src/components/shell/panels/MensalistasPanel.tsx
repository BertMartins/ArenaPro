...
export default function MensalistasPanel() {
  ...
  return (
    <div className="pb-24 animate-fadeIn max-w-2xl mx-auto">
      ...
      <div className="p-3 sm:p-5 space-y-4">
        <div className="glass-card rounded-xl p-4">
          ...
          <div className="flex gap-2 mb-3">
            ...
            <button onClick={saveFee} disabled={saving} className="btn-primary px-6 py-3 rounded-lg text-white font-bold">
              {saving ? "..." : "💾 Salvar"}
            </button>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-orange-500/20 border border-orange-500 rounded-lg p-3 text-center">
              <div className="text-lg font-bold text-orange-400">R$ {(data?.feePerPerson ?? 0).toFixed(2)}</div>
        ...
        {data?.monthlyUsers?.length > 0 ? (
            ...
            <div className="space-y-3">
              {data.monthlyUsers.map((u: any) => (
                <div key={u.id}
                  ...
                  <button onClick={() => togglePayment(u.id)}
                    className={`w-full py-2 rounded-lg text-white font-bold text-sm ${u.hasPaid ? "bg-red-600 hover:bg-red-700" : "bg-green-600 hover:bg-green-700"}`}>
          ...
          <div className="glass-card rounded-xl p-8 text-center">
            ...
            <button onClick={() => nav.pushView({ type: "users" })}
          ...
          <div className="bg-yellow-500/20 border border-yellow-500 rounded-xl p-4">
            <div className="flex gap-3">
              ...
              <div>
                ...
                <div className="text-yellow-100 text-sm">{pendingCount} mensalista{pendingCount > 1 ? "s ainda não pagaram" : " ainda não pagou"} este mês.</div>
              </div>
            </div>
          </div>
        )}
      ...
      {toast && (
        <div className={`fixed top-4 right-4 px-5 py-3 rounded-lg text-white z-[60] shadow-lg animate-slideIn ${toast.type === "error" ? "bg-red-500" : "bg-green-500"}`}>
          {toast.msg}
        </div>
      )}
    </div>
